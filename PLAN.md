...
- Completed:
  browser parity run-sheet artifact validation is tightened: `scripts/dashboard_browser_run_sheet_audit.php` now resolves recorded screenshot paths relative to the repository root and fails completed run sheets when referenced screenshot artifacts are missing, while `--allow-pending` still supports preflight handoff validation. The run-sheet and dated QA documentation now describe the screenshot path requirement.
- Suggested next task:
  execute the authenticated manual browser parity QA checklist against the seeded customer, employee, and admin accounts plus seeded workflow records, using `docs/dashboard-browser-parity-run-sheet.md` and the dated artifact folder from `scripts/prepare_dashboard_browser_qa_artifacts.php` to capture viewport-specific defects and screenshot notes. Record screenshot paths relative to the repository root, run `C:\xampp\php\php.exe scripts\dashboard_browser_run_sheet_audit.php` after filling the sheet so missing artifact paths are caught, then append confirmed findings to `docs/dashboard-parity-qa-2026-04-14.md`.
